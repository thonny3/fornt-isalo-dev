...
export default function TikoTable() {
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 5;
  const { getTikoTranfert, tiko } = useProduit();
  ...
  const rows = (tiko?.historique || []).flatMap((hist) =>
          ...
          <table className="min-w-full text-md text-left text-gray-500 bg-white border border-gray-300">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-300">
              <tr>
                {headers.map((header, index) => (
                  <th
                    key={index}
                    className="px-4 py-2 border border-gray-300 text-sm text-gray-500 font-normal"
                  >
            ...
            <tbody>
              {currentData.map((row, index) => (
                <tr
                  key={index}
                  className="hover:bg-gray-100 border-b border-gray-300 text-gray-600 mt-2"
                >
