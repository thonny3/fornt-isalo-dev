...
export default function TableTransactionBanque() {
  const [currentPage, setCurrentPage] = useState(1);
  const [pageGroup, setPageGroup] = useState(1);
  const itemsPerPage = 5;
  const {setOpenTransh, setEdit, ShowEditTrans, trans, setOpenDT } = useBanque();

  // Calculer le nombre total de pages
  const totalPages = Math.ceil((trans?.length || 0) / itemsPerPage);
  const pagesPerGroup = 3; // Nombre de pages affichées par groupe

  // Obtenir les données actuelles en fonction de la page
  const currentData = trans.slice(
      ...
      <div className="relative overflow-x-auto sm:rounded-lg">
        <table className="min-w-full text-md text-left text-gray-500 bg-white">
          <thead className="text-xs text-gray-700 uppercase bg-gray-50">
            <tr>
              <th className="px-4 py-2">Type Transaction</th>
              <th className="px-4 py-2">Montant</th>
              <th className="px-4 py-2">Date </th>
              <th className="px-4 py-2">Déscription </th>
            ...
            {currentData.map((row, index) => (
              <tr key={index} className="hover:bg-gray-100 ">
                <td className="px-4 py-4">
                  {row.type === 0 ? "Débit" : "Crédit"}
                </td>
                <td className="px-4 py-2">{row.montant}</td>
                <td className="px-4 py-2">{row.date}</td>
                <td className="px-4 py-2">{row.description==null ?"Null" : row.description }</td>
