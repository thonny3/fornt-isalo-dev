...
export default function DeleteBanque() {
  const {toast} =  useAdmin()
    const {data,setListBanque,onclose,openD,getAllBanque}=  useBanque()
  ...
  const deletePoste = () => {
    ...
    setIsLoading(true);
    Transaction.deleteBanque(data.id)
      .then(() => {
        toast.success("banque a été supprimé !");
        setListBanque((prevList) => prevList.filter((item) => item.id !== data.id)); // Mise à jour locale
        getAllBanque(); // Rechargement des données
        onclose();
      })
      .catch((error) => {
  ...
  return (
    <Modal open={openD} onClose={onclose}>
      <div className="modal-content mt-5">
        ...
        <p className="text-center font-semibold text-lg text-gray-700">
            {data && data.nom
              ? `Êtes-vous sûr de vouloir supprimer ce transaction  ${data.nom}?`
