    ...
    <Modal open={open} onClose={onclose}>
      <div className="header">
        <h1 className="text-lg font-semibold my-5 border-b border-gray-200 w-fit">
          {edit ? "Modifier" : "Création"} un  Catégorie
        </h1>
        <form >
          <div className="form ">
            {/* Colonne 1 */}
            <div className="form1">
              <div className="form-group">
                <label htmlFor="nom">
                  Nom du catégorie<span className="text-red-600 ml-2">*</span>
                </label>
                <input
                  type="text"
                  id="nom"
                  value={cat}
                  className={`form-control w-full ${errorsCat.cat && "border-red-600"}`}
                  placeholder="Entrer le nom du produit"
                  onChange={(e)=>setCat(e.target.value)}
                />
                {errorsCat.cat && (
                  <p className="text-red-600 mt-1 text-sm">{errorsCat.cat}</p>
                )}
              </div>
            </div>
          </div>
